...
const secondStyle = getValues(second);
const minuteStyle = getValues(minute);
...
.clock {
  --origin: calc(var(--size) * 0.9 / 2 - 2px); // dwa razy odjęte border
  position: relative;
  width: var(--size);
  height: var(--size);
  border-radius: 50%;
  ...
  .watch-face {
    ...
    .hand {
      ...
      rotate: 0deg;
      z-index: 2;
      animation: tick 60s steps(360, end) infinite;
